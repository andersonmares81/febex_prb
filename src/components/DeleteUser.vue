...
export default {
  ...
  methods: {
    cargarDatosUsuario() {
      if (this.userCod) {
        axios.get(this.$api_web_url + `/${this.userCod}`)
          .then(response => {
            console.log('Ver Usuario:', response.data.data);
  ...
  created() {
    if (this.$route.params.userCod) {
      this.cargarDatosUsuario();
    } else {
      console.error('userCod no definido');
    }
